...
base_url = 'https://klubok.com/sportivnaya-odezhda'
...
def parse_prom(base_url: str, headers: dict) -> list:
    ...
    if request.status_code == 200:
        ...
        cards = soup.find_all('section', attrs={'class': 'product-widget'})
        for card in cards:
            # Вытягиваем имя товара и превращаем в string
            name = card.find('p', attrs={'class': 'description'}).text

            # Вытягиваем актуальную цену. Если есть старая цена, то ищем по другому классу. Получаем str
            curr_price = card.find('span', attrs={'class': 'price'})
