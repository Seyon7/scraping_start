...
base_url = 'https://rozetka.com.ua/genskie-sportivnie-kostyumi/c4637319/'
...
def parse_prom(base_url: str, headers: dict) -> list:
    ...
    if request.status_code == 200:
        ...
        cards = soup.find_all('li', attrs={'class': 'catalog-grid__cell catalog-grid__cell_type_slim'})
        for card in cards:
            # Вытягиваем имя товара и превращаем в string
            name = card.find('span', attrs={'class': 'goods-tile__title'}).text

            # Вытягиваем актуальную цену. Если есть старая цена, то ищем по другому классу. Получаем str
            curr_price = card.find('span', attrs={'class': 'goods-tile__price-value'})
            ...
            old_price = card.find('div', attrs={'class': 'goods-tile__price goods-tile__price_type_old'})
            if old_price == '<!---->':
                old_price = 'None'
            else:
