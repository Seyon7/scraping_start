...
base_url = 'https://intertop.ua/catalog/zhenskaya_obuv/botinki/'
...
def parse_prom(base_url: str, headers: dict) -> list:
    ...
    if request.status_code == 200:
        ...
        cards = soup.find_all('div', attrs={'class': 'vue-one-product'})
        print(soup)
        for card in cards:
            # Вытягиваем имя товара и превращаем в string
            name = card.find('div', attrs={'class': 'product-name'}).text

            # Вытягиваем актуальную цену. Если есть старая цена, то ищем по другому классу. Получаем str
            curr_price = card.find('div', attrs={'class': 'current-price action_price'})
            ...
            old_price = card.find('div', attrs={'class': 'was-price'})
            if old_price == '<!---->':
                old_price = 'None'
            else:
