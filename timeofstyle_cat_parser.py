...
base_url = 'https://timeofstyle.com/zhenskaya-odezhda/majki-futbolki/'
...
def parse_prom(base_url: str, headers: dict) -> list:
    ...
    if request.status_code == 200:
        ...
        cards = soup.find_all('div', attrs={'class': 'product-block'})
        for card in cards:
            # Вытягиваем имя товара и превращаем в string
            name = card.find('div', attrs={'class': 'name'}).text

            # Вытягиваем актуальную цену. Если есть старая цена, то ищем по другому классу. Получаем str
            curr_price = card.find('span', attrs={'class': 'price-new'})
            if curr_price is None:
                curr_price = card.find('div', attrs={'class': 'price'}).text
            else:
            ...
            old_price = card.find('span', attrs={'class': 'price-old'})
