...
base_url = 'https://leboutique.com/catalog/men/jeans'
...
def parse_prom(base_url: str, headers: dict) -> list:
    ...
    if request.status_code == 200:
        ...
        cards = soup.find_all('div', attrs={'class': 'prod__desc'})
        for card in cards:
            # Вытягиваем имя товара и превращаем в string
            name = card.find('div', attrs={'class': 'prod__name crop'}).text

            # Вытягиваем актуальную цену. Если есть старая цена, то ищем по другому классу. Получаем str
            curr_price = card.find('div', attrs={'class': 'prod__price color_fire'})
            ...
            old_price = card.find('div', attrs={'class': 'prod__price-old'})
