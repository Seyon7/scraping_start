...
base_url = 'https://shoessale.com.ua/buy/female/boots/'
...
def parse_prom(base_url: str, headers: dict) -> list:
    ...
    if request.status_code == 200:
        ...
        cards = soup.find_all('div', attrs={'itemprop': 'itemListElement'})
        for card in cards:
            # Вытягиваем имя товара и превращаем в string
            name = card.find('span', attrs={'class': 'items__title'}).text

            # Вытягиваем актуальную цену. Если есть старая цена, то ищем по другому классу. Получаем str
            curr_price = card.find('span', attrs={'itemprop': 'price'})
