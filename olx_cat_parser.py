...
base_url = 'https://www.olx.ua/moda-i-stil/odezhda/muzhskaya-odezhda/?t=%D0%B4%D0%B6%D0%B8%D0%BD%D1%81%D1%8B'
...
def parse_prom(base_url: str, headers: dict) -> list:
    ...
    if request.status_code == 200:
        ...
        cards = soup.find_all('tr', attrs={'class': 'wrap'})
        for card in cards:
            # Вытягиваем имя товара и превращаем в string
            name = card.find('h3', attrs={'class': 'lheight22'}).text

            # Вытягиваем актуальную цену. Если есть старая цена, то ищем по другому классу. Получаем str
            curr_price = card.find('p', attrs={'class': 'price'})
            if curr_price is None:
                curr_price = card.find('span', attrs={'class': 'price'}).text
            else:
            ...
            # old_price = card.find('s', attrs={'class': 'price old'})
            ...
            data.append((name.strip(), curr_price.strip()))
        return data
    else:
        print('ERROR')
