...
base_url = 'https://issaplus.com/odezhda/sportivnyy-kostyum/'
...
def parse_prom(base_url: str, headers: dict) -> list:
    ...
    if request.status_code == 200:
        ...
        cards = soup.find_all('li', attrs={'class': 'producto col-md-3 col-lg-3 col-sm-4 col-xs-6'})
        for card in cards:
            # Вытягиваем имя товара и превращаем в string
            name = card.find('span', attrs={'itemprop': 'name'}).text

            # Вытягиваем актуальную цену. Если есть старая цена, то ищем по другому классу. Получаем str
            curr_price = card.find('span', attrs={'class': 'curent'})
            ...
            old_price = card.find('span', attrs={'class': 'old'})
