...
base_url = 'https://kasta.ua/market/zhenskie-sportivnye-kostyumy/'
...
def parse_prom(base_url: str, headers: dict) -> list:
    ...
    if request.status_code == 200:
        ...
        cards = soup.find_all('div', attrs={'class': 'product__item'})
        for card in cards:
            # Вытягиваем имя товара и превращаем в string
            name = card.find('div', attrs={'class': 'product__info_name'}).text

            # Вытягиваем актуальную цену. Если есть старая цена, то ищем по другому классу. Получаем str
            curr_price = card.find('span', attrs={'class': 'product_item__new-cost'})
            ...
            old_price = card.find('span', attrs={'class': 'product_item__old-cost'})
...
with open('kasta_cat_parse.txt', 'w') as file:
    for el in dataset:
