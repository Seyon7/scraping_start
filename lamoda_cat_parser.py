...
base_url = 'https://www.lamoda.ua/c/151/shoes-muzhskie-botinki/'
...
def parse_prom(base_url: str, headers: dict) -> list:
    ...
    if request.status_code == 200:
        ...
        cards = soup.find_all('a', attrs={'class': 'products-list-item__link link'})
        for card in cards:
            # Вытягиваем имя товара и превращаем в string
            name = card.find('div', attrs={'class': 'products-list-item__brand'}).text

            # Вытягиваем актуальную цену. Если есть старая цена, то ищем по другому классу. Получаем str
            curr_price = card.find('span', attrs={'class': 'price__new'})
            if curr_price is None:
                curr_price = card.find('span', attrs={'class': 'price__actual'}).text
            else:
            ...
            old_price = card.find('span', attrs={'class': 'price__old'})
