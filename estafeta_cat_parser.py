...
base_url = 'https://estafeta.com.ua/c5-krossovki_povsednevnie'
...
def parse_prom(base_url: str, headers: dict) -> list:
    ...
    if request.status_code == 200:
        ...
        cards = soup.find_all('div', attrs={'class': 'prod-in'})
        for card in cards:
            # Вытягиваем имя товара и превращаем в string
            name = card.find('span', attrs={'class': 'products-name'}).text

            # Вытягиваем актуальную цену. Если есть старая цена, то ищем по другому классу. Получаем str
            curr_price = card.find('span', attrs={'class': 'price sale'})
